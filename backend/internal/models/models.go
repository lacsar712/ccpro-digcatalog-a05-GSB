...
// CrewPerson 工地排班人员（非公司人事：仅记录工地用工称谓与角色）
...
// CrewShift 工地排班，挂靠 Site；同一人同工地同日唯一
type CrewShift struct {
	ID        uint           `json:"id" gorm:"primaryKey"`
	SiteID    uint           `json:"siteId" gorm:"not null;uniqueIndex:idx_crew_shift_unique"`
	WorkDate  time.Time      `json:"workDate" gorm:"type:date;not null;uniqueIndex:idx_crew_shift_unique"`
	PersonID  uint           `json:"personId" gorm:"not null;uniqueIndex:idx_crew_shift_unique"`
